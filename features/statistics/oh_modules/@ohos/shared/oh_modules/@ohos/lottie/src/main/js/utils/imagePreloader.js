...
const ImagePreloader = (function () {
  ...
  function generatePixelMapAndRefreshUI2(data, ob, callback) {
    ...
    try {
      image_source = image.createImageSource(data)
      ...
      // 捕获 createImageSource 的同步异常
      LogUtil.error('image.createImageSource 异常:', err);
      if (image_source) {
        image_source.release();
      }
      callback?.();
  ...
  // 独立的尺寸处理函数
  function handleImageDimensions(ob, callback) {
    if (ob.assetData?.w && ob.assetData?.h) {
      ob.img.width = Number(ob.assetData.w);
      ob.img.height = Number(ob.assetData.h);
      callback?.();
      return;
    }

    ob.img.pixel_map?.getImageInfo()
      .then((imageInfo) => {
        ob.img.width = imageInfo.size.width;
        ob.img.height = imageInfo.size.height;
        callback?.();
      })
      .catch((err) => {
        LogUtil.error('getImageInfo 失败:', err);
        callback?.();
      });
  }
  ...
  function readRawFile(context, assetData, ob, callback) {
    if (!context || !assetData || !ob) {
      LogUtil.warn('readRawFile: 参数无效');
      callback?.();
      return;
    }

    const res = assetData.u + assetData.p;
    const filePath = context.filesDir + '/' + res;
    let file = null;
    ...
    try {
      ...
      if (isExist) {
        // 抽取为独立函数，深度降低
        readFromFileSystem(filePath, ob, callback);
      } else {
        // 抽取为独立函数，深度降低
        readFromRawFile(context, res, ob, callback);
      }
    } catch (err) {
      LogUtil.error('readRawFile 执行异常:', err);
      callback?.();
    }
  }

  // ========== 独立的文件系统读取函数 ==========
  function readFromFileSystem(filePath, ob, callback) {
    let file = null;
    try {
      ...
      fs.read(file.fd, arrayBuffer, (err, readLen) => {
        ...
        if (!err && readLen > 0) {
          ...
          LogUtil.warn('readFromFileSystem: 读取失败', err);
          callback?.();
        }
        try {
          fs.closeSync(file);
      ...
      LogUtil.error('readFromFileSystem: 打开文件失败', openErr);
      if (file) {
        try {
          fs.closeSync(file);
        } catch (_) {
  ...
  // ========== 独立的 RawFile 读取函数 ==========
  function readFromRawFile(context, res, ob, callback) {
    context.resourceManager.getRawFileContent(res, (err, rawFileContent) => {
      // 深度现在只有 3 层
      if (!err && rawFileContent) {
        generatePixelMapAndRefreshUI2(rawFileContent.buffer, ob, callback);
      } else {
        LogUtil.warn('readFromRawFile: 获取失败', err);
        callback?.();
      }
    });
  }
