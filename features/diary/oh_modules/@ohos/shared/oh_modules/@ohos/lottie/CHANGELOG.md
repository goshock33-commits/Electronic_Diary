## v2.0.32
- fix: Optimize code robustness and fix potential memory leak issues that may occur under specific operational paths. [#364](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/364)

## v2.0.31
- fix: Fix Enhance code robustness to prevent exceptions when reading properties from null canvasShadow objects and accessing null dynamicProperties arrays. [#353](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/353)
- fix: Fix: Enhance code robustness and address issues related to reading properties from an empty `this.elements[i]` and reading image resources from an empty `assetData`. [#348](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/348)

## v2.0.30
- fix: Fixed the issue where getDuration() returned a duration unit that didn't match the original library when false, now uses frames/ms to calculate the duration, with the return value in ms. [#336](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/336)
- fix: Fix the loading failure issue of animations caused by the image width and height being of string type in JSON. [#337](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/337)
- fix: Fixed the bug where the internal destroy callback was mistakenly removed when calling `removeEventListener('destroy')` without passing a callback, resulting in a memory leak caused by the failure to release resources in `lottie.destroy()`. [#338](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/338)
- fix: Fix the issue of playback failure in Lottie animations caused by incompatibility with the circular interface of a higher version of the SDK. Implement compatibility between high and low versions through version isolation to ensure normal animation playback. [#340](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/340)
- fix: Fix the issue where Lottie animations fail to initialize due to missing resource attributes, which subsequently leads to a crash when accessing `this.layers.length` during destruction. [#341](https://gitcode.com/CPF-ApplicationTPC/lottieArkTS/pull/341)
